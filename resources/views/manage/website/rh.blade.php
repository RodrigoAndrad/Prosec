				...
				<b-collapse :open.sync="boxIsOpen">
	         	    ...
	         	    @foreach($boxes['elements'] as $box)
	         	    	<div class="columns panel-content p-l-20 p-t-20">
		            		...
		            		<!--
			            		<text-input-component :boxes="{{@json_encode($box['elements'])}}" placeholder-text="Digite aqui o título da página ..." name="boxTitulo{{$loop->iteration}}" label="Título"></text-input-component>
								<text-area-input-component :boxes="{{@json_encode($box['elements'])}}" placeholder-text="Digite aqui a descrição da página ..." name="boxTexto{{$loop->iteration}}" label="Texto"></text-area-input-component>
							-->
							</div>	
						</div>
	       		 	@endforeach
